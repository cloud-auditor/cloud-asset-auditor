...
// resolveRegions decides which regions to scan, following init-plan.md §3:
//   - empty list  → just the tenancy's home region (cheap default)
//   - {"all"}     → every region the tenancy is subscribed to
...
func (p *Provider) resolveRegions(ctx context.Context) ([]string, error) {
	...
	if len(want) == 0 {
		if p.homeRegion == "" {
			return nil, fmt.Errorf("no regions configured and home region is empty")
		}
		return []string{p.homeRegion}, nil
	}

	if len(want) == 1 && strings.EqualFold(want[0], "all") {
		return p.listSubscribedRegions(ctx)
	}
